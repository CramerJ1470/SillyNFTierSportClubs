...
contract GettingStartedFunctionsConsumer is FunctionsClient, ConfirmedOwner {
    ...
    string source =
        ...
        "   {url: 'https://heisenbug-premier-league-live-scores-v1.p.rapidapi.com/api/premierleague/table',method: 'GET',"
        "   headers: {'X-Rapidapi-Key': 'MYX_RAPIDAAPI_KEY','Content-Type'"
        "   :'application/json'}});"
        "if (request.error) {console.error('HTTP Request Failed:', request.error);throw new Error(`HTTP Request Failed: "
        "   ${request.error}`);}"
        "const standings = request.data.records || [];"
        "const filteredData = standings.map((team,index) => ({name:team.team, standing:index}));"
